#!/usr/bin/env python3
# =========================================================
# file name:    interactive_marker.py
# description:  update equilibrium pose, modified from franka_example_controllers
# author:       Xihan Ma
# date:         2024-02-29
# =========================================================
...
isUpdatePoseTeleop = False    # flag to resolve confliction between interactive marker and joystick teleop
...
def pose_update_callback(msg: Bool):
    global isUpdatePoseTeleop
    if isUpdatePoseTeleop != msg.data:
        # reset marker pose
        wait_for_initial_pose()
        server.insert(int_marker, process_feedback)
        server.applyChanges()
    isUpdatePoseTeleop = msg.data


def publisher_callback(msg, link_name):
    marker_pose.header.frame_id = link_name
    marker_pose.header.stamp = rospy.Time(0)
    if not isUpdatePoseTeleop:
...
if __name__ == "__main__":
    ...
    rospy.Subscriber("isUpdatePoseTeleop", Bool, pose_update_callback, queue_size=1)
    # run pose publisher
